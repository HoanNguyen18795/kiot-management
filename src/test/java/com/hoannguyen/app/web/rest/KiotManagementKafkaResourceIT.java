...
import com.hoannguyen.app.config.KafkaProperties;
...
class KiotManagementKafkaResourceIT {
    ...
    void setup() {
        ...
        KiotManagementKafkaResource kafkaResource = new KiotManagementKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/kiot-management-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/kiot-management-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
